// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:44:39; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::geometric_dist(double)
//
...
void geometric_dist_test(RNG<RNG_type> &r, long runs, long n, 
...
template<class RNG_type>
void geometric_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
			    long n, 
			    const vector<double> &borders,
			    const vector<double> &prob) {
  long j;
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    geometric_dist_test(r, runs, n, borders, prob);
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=5) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::geometric_dist(double)" << endl
      << "synopsis:" << endl    
      << "$ " << argv[0] << " runs sub_max n p" << endl 
      << endl
...
RNG r; geometric_dist_test_main(r, runs, sub_max, n, borders, prob);
