// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 23:35:01; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::uniform(void)
//
...
void uniform_test(RNG<RNG_type> &r, long runs, long n, long num_classes) {
  ...
  while (runs>0l) {
    ...
    for (i=0l; i<n; ++i)
      if (r.uniform())
	++observ[static_cast<int>(r.uniform()*num_classes)];
...
template<class RNG_type>
void uniform_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    uniform_test(r, runs, n, num_classes);
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=5) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::uniform(void)" << endl
      << "synopsis:" << endl    
      ...
      << "num_classes: number of urns" << endl 
      << endl
      << "The Programm throws n balls into num_classes urns and counts how" << endl
      << "many balls are thown into each urn and makes a Chi^2-test." << endl
      << endl
      << "just try" << endl
...
RNG r; uniform_test_main(r, runs, sub_max, n, num_classes);
