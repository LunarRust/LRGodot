// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:17:01; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::chi_squared_dist(double)
//
...
double p(double x) {
  return chi_square_dist_pdf(x, nu);
}
...
double integrate(double (*f)(double), double I, double x0, double dx) {
  ...
  while (I>0.0) {
    ...
    x0+=dx;
    y3=f(x0);
    I-=dx*(y1+4.0*y2+y3)/6.0;
  }
  return x0;
}

template<class RNG_type>
void chi_squared_dist_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.chi_square_dist(nu))];
...
template<class RNG_type>
void chi_squared_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
			long n, long num_classes,
			const vector<double> &borders) {
  long j;
#ifdef LaTeX
  cout << "\\nulticol{" << runs+1l <<"}{@{}l@{}}{Generator " << r.name() 
       << "}\\\\" << endl << "\\midrule" << endl;
#else
  ...
  for (j=1l; j<=sub_max; ++j) {
#ifdef LaTeX
    cout << "\\nulticol{" << runs+1l <<"}{@{}l@{}}{Splitting level " << j 
	 << "}\\\\" << endl;
#else
    ...
    r.reset();
    r.split(j, 0l);
    chi_squared_dist_test(r, runs, n, num_classes, borders);
  }
}
...
void calc_borders(vector<double> &borders, const long num_classes) {
  ...
  for (int j=0; j<num_classes-1; ++j) {
    x=integrate(p, 1.0/num_classes, x, 1.0/256.0/num_classes);
    borders[j]=x;
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=6) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::chi_squared_dist(double)" << endl
      << "synopsis:" << endl    
...
RNG r; chi_squared_dist_test_main(r, runs, sub_max, n, num_classes, borders);
