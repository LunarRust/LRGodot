// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:08:27; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::Beta_dist(double, double)
//
...
void Beta_dist_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.Beta_dist(a, b))];
...
template<class RNG_type>
void Beta_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    Beta_dist_test(r, runs, n, num_classes, borders);
  }
}
...
void calc_borders(vector<double> &borders, const long num_classes) {
  ...
  for (int j=0; j<num_classes-1; ++j) {
    x=integrate(p, 1.0/num_classes, x, 1.0/1024.0/num_classes/(a>b ? a : b));
    borders[j]=x;
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=7) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::Beta_dist(double)" << endl
      << "synopsis:" << endl    
      << "$ " << argv[0] << " runs sub_max n num_classes a b" << endl 
      << endl
      ...
      << endl
      << "just try" << endl
      << "$ " << argv[0] << " 8 128 4096 64 2.0 2.0" << endl;
    return EXIT_FAILURE;
  } else {
    try {
	...
	throw error("num_classes out of range");      
      a=atof(argv[5]);
      if (a<=0.0)
	throw error("a out of range");      
      b=atof(argv[6]);
      if (b<=0.0)
	throw error("b out of range");      
#ifdef LaTeX
...
RNG r; Beta_dist_test_main(r, runs, sub_max, n, num_classes, borders);
