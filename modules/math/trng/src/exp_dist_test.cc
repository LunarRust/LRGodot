// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:40:27; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::exp_dist(double)
//
...
void exp_dist_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.exp_dist(mu))];
...
template<class RNG_type>
void exp_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    exp_dist_test(r, runs, n, num_classes, borders);
  }
}
...
void calc_borders(vector<double> &borders, const long num_classes) {
  ...
  for (int j=0; j<num_classes-1; ++j) {
    x=integrate(p, 1.0/num_classes, x, mu/256.0/num_classes);
    borders[j]=x;
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=6) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::exp_dist(double)" << endl
      << "synopsis:" << endl    
      << "$ " << argv[0] << " runs sub_max n num_classes mu" << endl 
      << endl
...
RNG r; exp_dist_test_main(r, runs, sub_max, n, num_classes, borders);
