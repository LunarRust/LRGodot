// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:55:32; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::normal_dist(double, double)
//
...
double p(double x) {
  return normal_dist_pdf(x, sigma, mu);
}
...
void normal_dist_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.normal_dist(sigma, mu))];
...
template<class RNG_type>
void normal_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    normal_dist_test(r, runs, n, num_classes, borders);
  }
}
...
void calc_borders(vector<double> &borders, const long num_classes) {
  ...
  if (num_classes%2==0) {
    double x=mu;
    borders[num_classes/2-1]=mu;
    for (int j=1; j<num_classes/2; ++j) {
      x=integrate(p, 1.0/num_classes, x, 1.0/256.0/sigma/num_classes);
    ...
    double x=mu;
    x=integrate(p, 1.0/2.0/num_classes, x, 1.0/256.0/sigma/num_classes);
...
int main(int argc, char *argv[]) {
  if (argc!=7) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::normal_dist(double, double)" << endl
      << "synopsis:" << endl    
      << "$ " << argv[0] << " runs sub_max n num_classes mu sigma" << endl 
      << endl
      ...
      << endl
      << "just try" << endl
      << "$ " << argv[0] << " 8 128 4096 64 0.0 1.0" << endl;
    return EXIT_FAILURE;
  } else {
    try {
...
RNG r; normal_dist_test_main(r, runs, sub_max, n, num_classes, borders);
