// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 23:09:31; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method
// double TRNG::RNG::rejection(T_function f, double, double, double)
//
...
double integrate(t_function f, double I, double x0, double dx) {
...
void rejection_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.rejection(p(), -1.0, 1.0, 0.75))];
...
template<class RNG_type>
void rejection_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    rejection_test(r, runs, n, num_classes, borders);
  }
}
...
int main(int argc, char *argv[]) {
  if (argc!=5) {
    cerr 
      ...
      << endl
      << "testing method" << endl
      << "double TRNG::RNG::rejection(T_function f, double, double, double)" << endl
      << "synopsis:" << endl 
      << "$ " << argv[0] << " runs sub_max n num_classes" << endl 
      << endl
...
RNG r; rejection_test_main(r, runs, sub_max, n, num_classes, borders);
