// ---------------------------------------------------------------------
// Time-stamp: <Freitag, 24.01.2003, 22:52:57; edited by heiko>
// 
// Tina's random number generators TRNG
//
// testing random number generators 
// tests method  bool TRNG::RNG::laplace_dist(double)
//
...
void laplace_dist_test(RNG<RNG_type> &r, long runs, long n, long num_classes, 
  ...
  while (runs>0l) {
    for (i=0l; i<num_classes; ++i)
      observ[i]=0.0;
    for (i=0l; i<n; ++i)
      ++observ[find_interval(borders, r.laplace_dist(a))];
...
template<class RNG_type>
void laplace_dist_test_main(RNG<RNG_type> &r, long runs, long sub_max, 
  ...
  for (j=1l; j<=sub_max; ++j) {
    ...
    r.reset();
    r.split(j, 0l);
    laplace_dist_test(r, runs, n, num_classes, borders);
  }
}
...
void calc_borders(vector<double> &borders, const long num_classes) {
  ...
  if (num_classes%2==0) {
    ...
    for (int j=1; j<num_classes/2; ++j) {
      x=integrate(p, 1.0/num_classes, x, a/256.0/num_classes);
    ...
    double x=0.0;
    x=integrate(p, 1.0/2.0/num_classes, x, a/256.0/num_classes);
...
int main(int argc, char *argv[]) {
  if (argc!=6) {
    cerr 
      ...
      << endl
      << "testing method  bool TRNG::RNG::laplace_dist(double)" << endl
      << "synopsis:" << endl    
      << "$ " << argv[0] << " runs sub_max n num_classes a" << endl 
      << endl
	...
	throw error("num_classes out of range");      
      a=atof(argv[5]);
      if (a<=0.0)
	throw error("parameter a out of range");      
#ifdef LaTeX
...
RNG r; laplace_dist_test_main(r, runs, sub_max, n, num_classes, borders);
